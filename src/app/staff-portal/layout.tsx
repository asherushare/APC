...
function SidebarNav({ navItems, isMobile, onClose }: { navItems: NavItem[]; isMobile?: boolean; onClose?: () => void }) {
  ...
  const checkIsActive = (itemHref: string) => {
    ...
    if (pathname !== itemPath) {
      if (itemPath === '/staff-portal/applications' && pathname.startsWith('/staff-portal/applications/')) {
        return true;
      }
      return false;
    }
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    {
      label: 'Dashboard',
      href: '/staff-portal/dashboard?tab=stats',
      icon: (
  ...
  if (user?.role === 'ADMIN') {
    navItems.push({
      label: 'Coordinators',
      href: '/staff-portal/coordinators',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
