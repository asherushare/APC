...
class NodemailerProvider {
    ...
    async sendEmail(payload) {
        const mailOptions = {
            from: env_1.env.FROM_EMAIL || env_1.env.SMTP_FROM || '"APC Platform" <noreply@adivasiproducer.com>',
            to: payload.to,
...
let activeProvider;
const hasSmtpConfig = !!(env_1.env.SMTP_HOST && env_1.env.SMTP_USER && env_1.env.SMTP_PASS);
...
if (hasSmtpConfig || isProd) {
    try {
        ...
        logger_1.logger.error('❌ Failed to initialize SMTP Nodemailer. Falling back to Console Mail Mock.', error);
...
exports.emailService = {
    ...
    async sendApplicationStatusNotificationEmail(to, fullName, applicationId, status, reviewNotes) {
        ...
        switch (status) {
            ...
            case 'DOCUMENTS_PENDING':
                ...
                callToAction = `
          <div style="text-align: center; margin: 30px 0;">
            <a href="${env_1.env.FRONTEND_URL || 'http://localhost:3000'}/login" style="background-color: #d97706; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Update Documents</a>
          </div>
        `;
                break;
            case 'PAYMENT_PENDING':
                ...
                callToAction = `
          <div style="text-align: center; margin: 30px 0;">
            <a href="${env_1.env.FRONTEND_URL || 'http://localhost:3000'}/login" style="background-color: #047857; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Proceed to Payment</a>
          </div>
        `;
                break;
            case 'APPROVED':
      ...
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        ...
        <p>You can track your onboarding status anytime by logging in to the <a href="${env_1.env.FRONTEND_URL || 'http://localhost:3000'}/login" style="color: #047857; font-weight: bold; text-decoration: underline;">Public Producer Portal</a>.</p>
