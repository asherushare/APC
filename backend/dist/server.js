...
async function startServer() {
    try {
        ...
        try {
            ...
            logger_1.logger.error(`❌ Critical: Database connection check failed: ${err.message}. Exiting process immediately.`);
            process.exit(1);
        }
        // 2. Start HTTP server
        ...
        const err = error;
        logger_1.logger.error('❌ Failed to start server during initialization phase:');
        logger_1.logger.error(err.message || String(error));
        process.exit(1);
    }
}
startServer();
