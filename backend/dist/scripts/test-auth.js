...
function extractRefreshToken(setCookieHeaders) {
...
async function runTests() {
    ...
    const updatedUser = await db_1.prisma.user.findUnique({ where: { email: adminEmail } });
    ...
    const refreshAudit = await db_1.prisma.auditLog.findFirst({
    ...
    const activeTokens = await db_1.prisma.refreshToken.findMany({
    ...
    const reuseAudit = await db_1.prisma.auditLog.findFirst({
    ...
    const revokedToken = await db_1.prisma.refreshToken.findFirst({
        where: { tokenHash: (0, auth_1.hashRefreshToken)(activeRefreshToken) },
    });
    ...
    const logoutAudit = await db_1.prisma.auditLog.findFirst({
        where: { action: 'LOGOUT' },
    });
    if (logoutAudit) {
