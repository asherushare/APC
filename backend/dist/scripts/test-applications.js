...
async function runTests() {
    ...
    const existingAdmin = await db_1.prisma.user.findUnique({ where: { email: testAdmin.email } });
    if (!existingAdmin) {
    ...
    await db_1.prisma.shareholderApplication.deleteMany({});
    ...
    const dbRecord = await db_1.prisma.shareholderApplication.findUnique({
    ...
    const expectedHash = (0, crypto_1.hashAadhaar)(validPayload.aadhaarNumber);
    ...
    if (dbRecord.aadhaarEncrypted.includes(validPayload.aadhaarNumber) ||
    ...
    const docsCount = await db_1.prisma.document.count({ where: { applicationId: dbRecord.id } });
    if (docsCount !== 0) {
    ...
    // Verify audit log
    const audit = await db_1.prisma.auditLog.findFirst({
    ...
    const valError = await valRes.json();
    ...
    const dupError = await dupRes.json();
    ...
    if (listAdminData.applications.length !== 1)
        process.exit(1);
    ...
    const detailRayagadaData = (await detailRayagadaRes.json());
    const appDetails = detailRayagadaData.application;
    console.log('Checking decrypted details...');
    if (appDetails.aadhaarNumber !== validPayload.aadhaarNumber ||
