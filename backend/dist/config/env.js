...
const envSchema = zod_1.z.object({
    ...
    DATABASE_URL: zod_1.z.preprocess((val) => (typeof val === 'string' ? val.trim().replace(/^["']|["']$/g, '') : val), zod_1.z.string().url('DATABASE_URL must be a valid PostgreSQL connection URL')),
    ...
    SUPABASE_URL: zod_1.z.preprocess((val) => (typeof val === 'string' ? val.trim().replace(/^["']|["']$/g, '') : val), zod_1.z.string().url('SUPABASE_URL must be a valid URL')),
    ...
    FRONTEND_URL: zod_1.z.preprocess((val) => (typeof val === 'string' ? val.trim().replace(/^["']|["']$/g, '') : val), zod_1.z.string().url('FRONTEND_URL must be a valid URL')).default('http://localhost:3000'),
});
