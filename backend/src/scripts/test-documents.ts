...
async function runTests(): Promise<void> {
  ...
  const testAadhaarHash = crypto.createHash('sha256').update(testAadhaar).digest('hex');
  const existingApp = await prisma.shareholderApplication.findUnique({
    where: { aadhaarHash: testAadhaarHash },
  });
  if (existingApp) {
    await prisma.document.deleteMany({ where: { applicationId: existingApp.id } });
    await prisma.shareholderApplication.delete({ where: { id: existingApp.id } });
  }
  ...
  const appPayload = {
    ...
    bankBranch: 'Rayagada Main Branch',
  ...
  const appData = (await appRes.json()) as { success: boolean; applicationId: string; uploadToken: string };
  console.log(`✅ Application submitted! ID: ${appData.applicationId}`);
  console.log('🔑 uploadToken generated successfully.');

  // Find DB UUID
  const dbApp = await prisma.shareholderApplication.findUnique({
  ...
  async function testUpload(
    tokenHeader: { name: string; value: string } | null,
  ...
  await testUpload(
    null,
    401,
    'Applicant tries to upload without token (should fail with 401)'
  );

  // TEST 3: Applicant tries to upload with invalid token (Fail 403)
  await testUpload(
    { name: 'X-Upload-Token', value: 'invalid_token_value_here' },
    403,
    'Applicant tries to upload with invalid token (should fail with 403)'
  );

  // TEST 4: Rayagada Coordinator uploads document (Pass since block matches)
  await testUpload(
    { name: 'Authorization', value: `Bearer ${rayagadaToken}` },
    201,
    'Rayagada Coordinator uploads document (block matches application, should pass)'
  );

  // TEST 5: Muniguda Coordinator tries to upload document (Fail 403 since block is different)
  await testUpload(
    { name: 'Authorization', value: `Bearer ${munigudaToken}` },
    403,
    'Muniguda Coordinator tries to upload document (block mismatch, should fail with 403)'
  );
  ...
  await testUpload(
    ...
    201,
    'Admin uploads document (unrestricted role, should pass)'
  );
  ...
  await testUpload(
    { name: 'X-Upload-Token', value: appData.uploadToken },
    422,
    'Upload invalid file format (.html) (should fail with 422)',
    false
  );
  ...
  const verifiedDoc = await prisma.document.findUnique({
    where: { id: docId },
  });
