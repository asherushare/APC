...
async function runTests(): Promise<void> {
  ...
  const existingApps = await prisma.shareholderApplication.findMany({
  ...
  if (appIds.length > 0) {
    ...
    await prisma.producerActivity.deleteMany({ where: { applicationId: { in: appIds } } });
    await prisma.shareholderApplication.deleteMany({ where: { id: { in: appIds } } });
  }

  // 3. Create test applications
  console.log('📝 Seeding test applications...');
  const appRayagada = await prisma.shareholderApplication.create({
    data: {
      ...
      nomineeDateOfBirth: new Date('1995-01-01'),
  ...
  const appMuniguda = await prisma.shareholderApplication.create({
    data: {
      ...
      nomineeDateOfBirth: new Date('2015-05-05'),
  ...
  const body1 = (await res1.json()) as { success: boolean; application: { reviewedAt: string | null } };
  ...
  const statsBodyAdmin = (await statsResAdmin.json()) as { success: boolean; stats: Record<string, number> };
  ...
  const statsBodyRayagada = (await statsResRayagada.json()) as { success: boolean; stats: Record<string, number> };
  ...
  const auditBodyAdmin = (await auditResAdmin.json()) as { success: boolean; logs: Array<{ targetId: string; userId: string; action: string }> };
  ...
  const auditBodyRayagada = (await auditResRayagada.json()) as { success: boolean; logs: Array<{ targetId: string; userId: string; action: string }> };
